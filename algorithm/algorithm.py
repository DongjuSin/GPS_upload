...
from gps.utility.data_logger import DataLogger # Alisher, dyn
...
class Algorithm(object):
    ...
    def __init__(self, hyperparams):
        ...
        """
        Modification for using existing dynamics and lin controller objects
        Alisher 
        """
        
        self.data_logger = DataLogger()
        self.use_prev_dyn = False
        if self.use_prev_dyn:
            self._data_files_dir ='/hdd/gps-master/experiments/prepare_controller/data_files/'
            algorithm_file = self._data_files_dir + 'dynamics.pkl'
            self.algorithm_for_dyn = self.data_logger.unpickle(algorithm_file)
            
            if self.algorithm_for_dyn is None:
                print "Error: cannot find '%s.'" % algorithm_file
                exit()
            print "%s is loaded to initialze local controller" % algorithm_file

            for m in range(self.M):
                self.cur[m].traj_info = TrajectoryInfo()
                self.cur[m].traj_info = self.algorithm_for_dyn[m].traj_info
                self.cur[m].traj_distr = self.algorithm_for_dyn[m].traj_distr
        else:       
            for m in range(self.M):
                self.cur[m].traj_info = TrajectoryInfo()
                if self._hyperparams['fit_dynamics']:
    ...
    def _update_dynamics(self):
        ...
        for m in range(self.M):
            ...
            self.cur[m].traj_info.dynamics.update_prior(cur_data, m, self.iteration_count)
            self.cur[m].traj_info.dynamics.fit(X, U)
            
            #self.cur[m].traj_info.dynamics.save_fitted_dynamics(X, U, m, self.iteration_count)
    ...
    def _update_trajectories(self, itr):
        """
        Compute new linear Gaussian controllers.
        """
        ## modified by dongju : to prevent update of controller
        '''
        if True:
            self.new_traj_distr = [
        ...
        for cond in range(self.M):
            self.new_traj_distr[cond], self.cur[cond].eta = \
                    self.traj_opt.update(itr, cond, self)
