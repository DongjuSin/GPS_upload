...
class DynamicsPriorGMM(object):
    ...
    def update(self, X, U, cond, itr):
        ...
        np.savez('./plot_dynamics/data/gmm_parameter_' + str(itr) + '_' + str(cond) + '.npz', \
                            K=K, N=N, T=T, \
                            max_clusters=self._max_clusters, \
                            min_sample=self._min_samp)

        # Update GMM.
        self.gmm.update(xux, K, cond=cond, algo_itr=itr)
