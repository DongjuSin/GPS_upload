...
class TrajOptLQRPython(TrajOpt):
    ...
    def update(self, iteration, m, algorithm):
        ...
        # np.savez('./policy_update/traj_update.npz', traj_distr=traj_distr, eta=eta)
    ...
    def backward(self, prev_traj_distr, traj_info, eta, algorithm, m, itr):
        """
        ...
        """
        ## dongju : check terms consists covariance
        Q_hessian = []
        mult_list = []
        fu_list = []
        Vxx_list = []
        ...
        while fail:
            ...
            for t in range(T - 1, -1, -1):
                ...
                ## dongju : check terms consists covariance
                Q_hessian.append(traj_distr.pol_covar[t, :, :])
                if t < T-1:
                    mult_list.append(multiplier)
                    fu_list.append(Fm[t, :, idx_u])
                    Vxx_list.append(Vxx[t+1, :, :])
                ##
        ...
        ## dongju : check terms consists covariance
        Q_hessian = np.asarray(Q_hessian)
        mult_list = np.asarray(mult_list)
        fu_list = np.asarray(fu_list)
        Vxx_list = np.asarray(Vxx_list)
        
        path = '/hdd/gps-master/experiments/' + 'baxter_kiro' + '/data_files/check_cntr_covar'
        if not os.path.exists(path):
            os.mkdir(path)
            print path, ' is created'
        fname = path + '/controller_covar_%d_%d.npz' % (itr, m)
        np.savez_compressed(fname, Q_hessian=Q_hessian, mult_list=mult_list, fu_list=fu_list, Vxx_list=Vxx_list)
        ##
