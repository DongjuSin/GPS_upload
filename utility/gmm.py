...
class GMM(object):
    ...
    def update(self, data, K, max_iterations=100, cond=None, algo_itr=None):
        """
            ...
            for i in range(K):
                ...
                if self.eigreg:  # Use eigenvalue regularization.
                    ...
                    self.sigma[i, :, :] = 0.5 * (sigma + sigma.T) + \
                            1e-6 * np.eye(Do)
        '''
        if cond is not None and algo_itr is not None:
            np.savez('./plot_dynamics/data/dynamics_' + str(algo_itr) + '_' + str(cond) + '.npz', data=data, argmax_w=np.argmax(w, axis=1), mu=self.mu, sigma=self.sigma)
        '''